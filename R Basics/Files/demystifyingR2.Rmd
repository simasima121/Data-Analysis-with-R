...
more code.
```{r}
mtcars$wt # setting value to mtcars weight column
cond <- mtcars$wt < 3 # setting cond to boolean True/False if mtcars$weight < 3
cond
mtcars$weight_class <- ifelse(cond, 'light', 'average') # setting weight_class to either light or average depending on whether the cond was true or false
mtcars$weight_class
cond <- mtcars$wt > 3.5 # setting cond to True/False if weight > 3.5
mtcars$weight_class <- ifelse(cond, 'heavy', mtcars$weight_class) # set to heavy, or remain the same depending on whether cond was true or false
mtcars$weight_class
```
