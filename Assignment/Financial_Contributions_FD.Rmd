...
### Setting global options for the plots
...
There was a suprising relationship between Candidates and Time. 
...
This plot is used to visualise the frequency of money being contributed. 
...
### Contribution Amounts and Redesignate/Refunds by Gender
...
Female candidates had almost 3 times higher total contributions, 3 times more
mean contributions and larger median contributions than males. This was a little 
...
It was clear that females had a much higher ratio of contribution amount to 
refund amount than males yet had almost the same ratio of refunds to 
contributions, however only <1% of total contributions were refunded for females 
compared ~10% for males. 

This led me to believe that large contributors were taking back larger portions 
of their money for males compared to females. I did a plot of the actual refunds
and found that for females, there were many more refunds at lower values. This 
further validates the theory that larger donors were Redesignate/Refunding money
from male candidates whereas smaller donors were Redesignate/Refunding money 
from females.

## Date and Contributions
...
The contribution amounts increased with every month until April 2016 (when the 
Primary Elections were decided) and have remained high since.

## Geography and Contributions
...
ggplot() +
  ...
  geom_point(data = subset(us.cities,
             ...
             aes(x = long,
                 ...
                 size = pop),
             colour = "green") +
  labs(colour = "Amount", size = "Population") + 
  geom_text(data = subset(us.cities,
                          us.cities$country == "NY" & us.cities$pop > 100000),
            aes(x = long,
                y = lat,
            ...
            colour = "gold",
            fontface="bold") +
  scale_colour_gradientn(colours = rainbow(70))
```

As can be seen, most donations were of the same size and didnt vary much by
...
The relationship between contribution frequency and zip was clear, with most
contributions coming from heavily populated areas such as cities, however the
relationship between zip and contribution amount was not clear.
...
Green partys 238 (1585 times more).
...
### Contribution Amounts and Redesignate/Refunds by Party
...
merged_party <- merge(mdata.by_party,
                      mdata.by_party_refunds, 
                      by=c("party"), all.x = TRUE)
...
### Contribution Amounts and Redesignate/Refunds by Candidate
...
merged_candidate <- merge(mdata.by_candidate, 
                          mdata.by_candidate_refunds, 
                          by=c("cand_nm", "party"), all.x = TRUE)
...
p1 <- ggplot(subset(merged_candidate, merged_candidate$c_amount_sum > 1000000),
             aes(cand_nm, c_amount_mean, fill = party)) + 
  geom_bar(stat="identity") +
  xlab("Candidate") +
  ylab("Mean Contribution") + 
...
p2 <- ggplot(subset(merged_candidate, merged_candidate$c_amount_sum > 1000000),
             aes(cand_nm, c_amount_median, fill = party)) + 
  geom_bar(stat="identity") +
  xlab("Candidate") +
  ylab("Median Contribution") +
...
As we were focused on candidates with more than $1MM in contributions,
...
p1 <- ggplot(subset(merged_candidate, merged_candidate$c_amount_sum > 1000000), 
             aes(cand_nm, total_ratio, fill = party)) + 
  geom_bar(stat="identity") +
  xlab("Party") +
  ylab("Ratio") +
...
p2 <- ggplot(subset(merged_candidate, merged_candidate$c_amount_sum > 1000000),
             aes(cand_nm, count_ratio, fill = party)) + 
  geom_bar(stat="identity") +
  xlab("Party") +
  ylab("Ratio") +
...
p1 <- ggplot(subset(merged_candidate, merged_candidate$party == "R"), 
             aes(cand_nm, c_count, fill = cand_nm)) + 
  geom_bar(stat="identity") +
  xlab("Candidate") +
...
p2 <- ggplot(subset(merged_candidate, merged_candidate$party == "D"), 
             aes(cand_nm, c_count, fill = cand_nm)) + 
  geom_bar(stat="identity") +
  xlab("Candidate") +
  ylab("Number of Contributions") +
...
p3 <- ggplot(subset(merged_candidate, merged_candidate$party == "R"), 
             aes(cand_nm, c_amount_sum, fill = cand_nm)) + 
  geom_bar(stat="identity") +
  xlab("Candidate") +
  ylab("Total Contributions") +
...
p4 <- ggplot(subset(merged_candidate, merged_candidate$party == "D"), 
             aes(cand_nm, c_amount_sum, fill = cand_nm)) + 
  geom_bar(stat="identity") +
  xlab("Candidate") +
  ylab("Total Contributions") +
...
## Contributions by Candidate and Time
...
### Plot One
```{r echo=FALSE, warning=FALSE, message=FALSE, error=FALSE, Plot_One}
qplot(x = amount, y = ..count..,
      data = subset(mdata,  amount > 0), 
...
stages. This took me by suprise and invigorated me to investigate features such
as candidates, gender, zipcode and time to see if these could explain it. 

### Plot Two
```{r echo=FALSE, warning=FALSE, message=FALSE, error=FALSE, Plot_Two}
ggplot() +
  geom_polygon(data = newyork,
               aes(x = long, y = lat, group = group)) +
  geom_jitter(aes(x = longitude, y = latitude, group = NULL, color = party), 
  ...
  geom_text(data = subset(us.cities,
            ...
            colour = "gold",
            fontface="bold") +
  ggtitle("Contributions by Zipcode to Party's") +
  guides(colour = guide_legend(override.aes = list(alpha = 1))) +
  labs(color = "Party", size = "Population")
```
...
The NY Contributions dataset contained information on almost 430000 
contributions with 19 features. To begin the analysis, I started by understanding
the individual variables, deciding which variables were useful or not and 
removing or fetching variables as needed. I merged the contribution dataset with
a zipcode dataset aswell as a gender and party dataset to get all the info I 
needed. Once this was done, I was able to explore the most pertinent questions 
and I made observations on plots. Eventually, I explored many variables involved
in contributions and got a deep understanding of how contributions change over 
an election.
...
of the contributions and this made it somewhat difficult to glean further insight.
...
and news data to see whether contributions varied on headline stories or by 
adding another feature in votes.
