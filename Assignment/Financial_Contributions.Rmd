...
#set.seed(1836)
#head(mdata[,c(1:2,9:10,18,22:23)])

#m_subset <- mdata[,c(1:2,9:10,18,22:23)]
#names(m_subset)
#ggpairs(m_subset[sample.int(nrow(m_subset), 20), ])
```


### Party Contributions and Refunds

```{r echo=FALSE, Creating table for party}
# Without Refunds
...
#### Contributions
```{r echo=FALSE, Party Contributions}
...
p1 <- ggplot(mdata.by_party, aes(party, amount_sum, fill = party)) + 
  ...
  ggtitle("2016 US Election Contribution by Parties") +
...
### Candidate Contributions and Refunds

```{r echo=FALSE, Creating table for Candidates}
mdata.by_candidate <- mdata %>%
  group_by(cand_nm) %>%
  summarise(amount_sum = sum(amount),
            amount_mean = mean(amount),
            amount_median = median(amount),
...
#### Contributions
```{r echo=FALSE, Candidate Contributions}

```

#### Refunds
```{r echo=FALSE, Candidate Refunds}

```

### Gender Contributions and Refunds

```{r echo=FALSE, Creating table for party}
mdata.by_gender <- mdata %>%
  group_by(gender) %>%
  summarise(amount_sum = sum(amount),
            amount_mean = mean(amount),
            amount_median = median(amount),
...
#### Contributions
```{r echo=FALSE, Gender Contributions}

```

#### Refunds
```{r echo=FALSE, Gender Refunds}
