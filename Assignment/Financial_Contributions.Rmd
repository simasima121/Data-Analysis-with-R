...
From the plot, you can visualise the distribution of donations from NY State, however the boundaries could be more clear. I used a [tutorial on ggplot](https://uchicagoconsulting.wordpress.com/tag/r-ggplot2-maps-visualization/) to achieve this.
...
ggplot() +
  ...
  geom_text(data = subset(us.cities,
            ...
            fontface="bold") 
```
From this data, you can see that the large cities have the highest frequency of contributions.


From the initial plot, you can see there are negative amounts being displayed. This is because of refunds. There were 4722 refunds and we will explore them in further detail later.

After removing the refunds, we see a positive skew in the distribution, with most of the money contributed between 15 to 100 Dollars. The median contribution is 27 with an IQR of 85. The mean is quite heavily skewed (probably due to v. large contributions).

With the plot transformed using log, the distribution is normal with a peark around
```{r }
...
# Summary with/without refunds.
summary(mdata$amount)
summary(subset(mdata$amount, mdata$amount >= 0))

qplot(x = amount,
      data = subset(mdata, amount > 0 & amount < 10000),
      binwidth = 0.2) +
  scale_x_log10()
