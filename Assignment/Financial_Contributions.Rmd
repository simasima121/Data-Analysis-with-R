...
```{r echo=FALSE, Cntbtn_By_P_and_C}
...
I wanted to see how the contribution varied over time for the top 4 most contributed candidates.

From tabling and plotting the data, we can see that contributions started coming regularly from April 2015. 

```{r echo=FALSE, Top_4_Candidates}
...
dates <- as.Date(top_cont$date, "%d-%b-%Y")

top_cont$day <- as.numeric(format(dates, format = "%d"))
top_cont$month <- as.numeric(format(dates, format = "%m"))
top_cont$year <- as.numeric(format(dates, format = "%Y"))

# Seeing how many donations by year
table(top_cont$year,top_cont$month)

# Plotting by year
ggplot(data = top_cont, aes(x = month)) + 
